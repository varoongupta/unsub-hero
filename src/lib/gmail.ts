...
export async function makeOAuth(clerkId: string): Promise<OAuth2Client> {
  ...
  const token = await prisma.gmailToken.findFirst({ where: { userId: user.id }, orderBy: { createdAt: "desc" } });
...
export async function listTopSenders(auth: OAuth2Client) {
  ...
  const aggregates = new Map<string, { id: string; from: string; fromEmail: string; listUnsub?: string; count: number; hasHttp: boolean; hasMailto: boolean }>();
  ...
  for (let i = 0; i < messages.length; i += batchSize) {
    ...
    const batch = await Promise.all(
      slice.map(async (id): Promise<{ id: string; from: string; fromEmail: string; listUnsub?: string } | null> => {
        const msg = await gmail.users.messages.get({ userId: "me", id, format: "metadata", metadataHeaders: ["From", "List-Unsubscribe"] });
    ...
    for (const item of batch) {
      ...
      if (!prev) {
        aggregates.set(key, { id: item.id, from: item.from, fromEmail: item.fromEmail, listUnsub: item.listUnsub, count: 1, hasHttp, hasMailto });
      } else {
        prev.count += 1;
